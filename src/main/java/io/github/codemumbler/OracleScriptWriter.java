...
public class OracleScriptWriter {
  ...
  private static final List<String> RESERVED_WORDS = Arrays.asList("SQL", "GROUP", "LANGUAGE", "BY", "DECLARE", "SELECT", "WHERE", "FROM", "NUMBER");
  ...
  private String additionalUpdates(Row row) {
    ...
    for (Column column : row.getColumns()) {
      ...
      builder.append(String
          .format(CLOB_INSERTION, column.getDataType().writeValue(row.get(column)), cleanName(row.getTableName()), cleanName(column.getName()),
  ...
  private String buildSetWhereClause(Table table, Row row) {
    ...
    for (Column column : row.getColumns()) {
      if (column.getDataType().isInsertable())
        whereClause.append(cleanName(column.getName())).append(" = ").append(column.getDataType().writeValue(row.get(column))).append(" AND ");
    }
  ...
  public String writeForeignKey(Table table) {
    ...
    for (ForeignKey foreignKey : table.getForeignKeys()) {
      builder.append(String.format(FOREIGN_KEY, cleanName(table.getName()), foreignKeyIndex++, cleanName(foreignKey.getChildColumn().getName()),
          cleanName(foreignKey.getParentTable().getName()), cleanName(foreignKey.getParentColumn().getName())));
    }
    return builder.toString();
  }
